...
LOCAL_ADMIN_PASSWORD="0v4kmNI5iixf"

echo "==> [1/12] cloud-init State löschen..."
cloud-init clean --logs --seed

echo "==> [2/12] Machine-ID zurücksetzen..."
truncate -s 0 /etc/machine-id
rm -f /var/lib/dbus/machine-id
ln -s /etc/machine-id /var/lib/dbus/machine-id

echo "==> [3/12] SSH Host Keys löschen..."
rm -f /etc/ssh/ssh_host_*

echo "==> [4/12] Netplan cloud-init Config entfernen (Fallback bleibt erhalten)..."
rm -f /etc/netplan/50-cloud-init.yaml

echo "==> [5/12] Alle lokalen User ausser '${LOCAL_ADMIN_USER}' entfernen..."
...
echo "==> [6/12] '${LOCAL_ADMIN_USER}' sicherstellen..."
...
echo "==> [9/12] SSSD stoppen & deaktivieren (wird nach realm join aktiviert)..."
systemctl disable sssd 2>/dev/null || true
systemctl stop sssd 2>/dev/null || true

echo "==> [10/12] Shell History & Temp-Dateien löschen..."
truncate -s 0 /root/.bash_history
truncate -s 0 /home/*/.bash_history 2>/dev/null || true
rm -rf /tmp/* /var/tmp/*

echo "==> [11/12] Package Cache & Logs bereinigen..."
apt autoremove -y && apt clean
journalctl --rotate
journalctl --vacuum-time=1s
find /var/log -type f -exec truncate -s 0 {} \;

echo "==> [12/12] SSSD Cache wird bewusst behalten..."
