...
# ================================================================
log "Pakete installieren (Apache, MariaDB, PHP ${PHP_VER})"

export DEBIAN_FRONTEND=noninteractive

apt-get update

# Sicherstellen, dass die Distro tatsaechlich php${PHP_VER} liefert.
# 26.04 bringt PHP 8.5 nativ mit; auf anderer Basis PHP_VER oben anpassen.
if ! apt-cache show "php${PHP_VER}" >/dev/null 2>&1; then
    echo "    FEHLER: Paket 'php${PHP_VER}' nicht in den Paketquellen gefunden."
    echo "    Diese Ubuntu-Basis liefert eine andere PHP-Version."
    echo "    PHP_VER oben im Script auf die vorhandene, von Nextcloud"
    echo "    unterstuetzte Version (8.3/8.4/8.5) setzen."
    exit 1
fi
...
# OPcache: bis PHP 8.3 (Ubuntu 24.04) ein eigenes Paket 'php8.3-opcache';
# ab der 26.04-Paketierung (PHP 8.5) gibt es kein eigenes Paket mehr — das
# Modul steckt in 'php${PHP_VER}-common' und wird als Abhaengigkeit von
# php${PHP_VER} automatisch mitinstalliert und per Default aktiviert.
# Daher nur explizit anfordern, wenn das Paket in den Quellen existiert.
if apt-cache show "php${PHP_VER}-opcache" >/dev/null 2>&1; then
    PKGS+=("php${PHP_VER}-opcache")
fi

apt-get install -y "${PKGS[@]}"

echo "    Alle Pakete installiert (PHP ${PHP_VER})."
